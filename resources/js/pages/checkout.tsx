...
export default function Checkout({
  ...
  const selectedShipping = useMemo(
    () =>
      localShippingOptions.find((o) => `${o.courier}-${o.service}` === shipKey),
    [shipKey, localShippingOptions],
  );
  const shippingCost = selectedShipping?.cost ?? 0;
  const total = subtotal + shippingCost;
  const totalWeight = useMemo(() => items.length * 1000, [items]);
  useEffect(() => {
    const addr = addresses.find((a) => a.id.toString() === addressId);
    if (!addr) return;
    fetch(`/shipping/options?city_id=${addr.city_id}&weight=${totalWeight}`)
      .then((r) => r.json())
      .then(setShippingOptions)
      .catch(() => toast.error('Gagal memuat ongkir.'));
  }, [addressId]);
  function bayar() {
